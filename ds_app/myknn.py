...
class CKNN:
    ...
    def __init__(self,id):
        ...
        con = pymysql.connect(host="localhost", port=3306, user="root", password="12345678", db="disease prediction")
        cmd = con.cursor()
        cmd.execute("select distinct symptom from ds_app_symptoms WHERE `disease_id_id` IN("+id+") order by symptom")
        s = cmd.fetchall()
        dsym = []
        for i in s:
            dsym.append(i[0])
        cmd.execute("select * from ds_app_disease where id in ("+id+")")
        s = cmd.fetchall()
        dataset = []
        ansset = []
        disdis=[]
        for i in s:
            cmd.execute("select distinct symptom from ds_app_symptoms  where disease_id_id=" + str(i[0]))
            ss = cmd.fetchall()
            myrow = []
            for ii in ss:
                myrow.append(ii[0])
